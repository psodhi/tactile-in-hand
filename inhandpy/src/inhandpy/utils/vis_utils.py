...
class Visualizer3d:
    def __init__(self, base_path='', view_params=None, tsleep=0.01):
        ...
        self.view_params = AttrDict({'fov': 0, 'front': [0., 0., 0.], 'lookat': [
                                    0., 0., 0.], 'up': [0., 0., 0.], 'zoom': 0.5})
    ...
    def _init_options(self):
        ...
        opt.background_color = np.asarray([.6, .6, .6])
    
        # pause option
        self.paused = types.SimpleNamespace()
        self.paused.value = False
        self.vis.register_key_action_callback(ord('P'), lambda a,b,c: b == 1 or setattr(self.paused, "value", not self.paused.value))

    def _init_geom_cloud(self):
        return o3d.geometry.PointCloud()

    def _init_geom_frame(self, frame_size=0.01, frame_origin=[0, 0, 0]):
        return o3d.geometry.TriangleMesh.create_coordinate_frame(size=frame_size, origin=frame_origin)

    def _init_geom_mesh(self, mesh_name, color=None, wireframe=False):
        mesh = o3d.io.read_triangle_mesh(f"{self.base_path}/local/resources/meshes/{mesh_name}")
        mesh.compute_vertex_normals()

        if wireframe: mesh = o3d.geometry.LineSet.create_from_triangle_mesh(mesh)
        if color is not None: mesh.paint_uniform_color(color)

        return mesh

    def init_geometry(self, geom_type, num_items=1, sizes=None, file_names=None, colors=None, wireframes=None):
        geom_list = []
        for i in range(0, num_items):
            ...
            if (geom_type == "cloud"):
                ...
                log.error(f"[Visualizer3d::init_geometry] geom_type {geom_type} not found.")
    ...
    def transform_geometry_relative(self, transform_prev_list, transform_curr_list, geom_list):
        for idx, geom in enumerate(geom_list):
            T_prev = transform_prev_list[idx]
            T_curr = transform_curr_list[idx]

            # a. rotate R1^{-1}*R2 about center t1
            geom.rotate(torch.matmul(torch.inverse(T_prev[0:3, 0:3]), T_curr[0:3, 0:3]), center=(T_prev[0,-1], T_prev[1,-1], T_prev[2,-1]))

            # b. translate by t2 - t1
            geom.translate((T_curr[0, -1]-T_prev[0, -1], T_curr[1, -1]-T_prev[1, -1], T_curr[2, -1]-T_prev[2, -1]))
...
def visualize_geometries_o3d(vis3d, clouds=None, frames=None, meshes=None, transforms=None):

        if meshes is not None: 
            meshes = [copy.deepcopy(mesh) for mesh in meshes]
            if transforms is not None: vis3d.transform_geometry_absolute(transforms, meshes)
        
        if frames is not None: 
            frames = [copy.deepcopy(frame) for frame in frames]
            if transforms is not None: vis3d.transform_geometry_absolute(transforms, frames)

        if clouds is not None: vis3d.add_geometry(clouds)
        if meshes is not None: vis3d.add_geometry(meshes)
        if frames is not None: vis3d.add_geometry(frames)

        vis3d.render()

        if clouds is not None: vis3d.remove_geometry(clouds)
        if meshes is not None: vis3d.remove_geometry(meshes)
        if frames is not None: vis3d.remove_geometry(frames)
...
def depth_to_color(depth):
    gray = (np.clip((depth-depth.min()) / (depth.max() - depth.min()), 0, 1) * 255).astype(np.uint8)
    return cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)

def visualize_flow_cv2(img1, img2, flow_arrow=None, flow_color=None, win_size=(360, 360)):
...
def draw_rectangle(center_x, center_y, size_x, size_y, ang=0., edgecolor='dimgray',
    ...
    rect = Rectangle((anchor_x, anchor_y), size_x, size_y, angle=(np.rad2deg(ang)),
...
def visualize_imgs(fig, axs, img_list, titles=None, cmap=None):
    ...
    for idx, img in enumerate(img_list):
        ...
        if cmap is not None: fig.colorbar(im, ax=axs[idx])
        if titles is not None: axs[idx].set_title(titles[idx])